...
public static class OpenAIClientExtensions
{
    public static IServiceCollection AddOpenAIClient(this IServiceCollection services, IConfigurationSection configuration)
    {
        ...
        {
            // Uri is empty -> use OpenAI
            client = new OpenAIClient(options.ApiKey);
        }
        else
        {
            // Uri is not empty -> use Azure OpenAI
            client = new OpenAIClient(new Uri(options.Uri), new AzureKeyCredential(options.ApiKey));
        }
...
public class OpenAISettings
{
    ...
    /// <summary>
    /// The deployment name for the OpenAI client.
    /// </summary>
    /// <remarks>
